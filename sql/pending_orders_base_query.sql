SELECT
  ...
  END AS "Order Type",
  CASE
    WHEN EXTRACT(EPOCH FROM (NOW() - o."createdAt")) / 3600 <= 12 
      THEN 'Fresh Order (0-12 hrs)'
    WHEN EXTRACT(EPOCH FROM (NOW() - o."createdAt")) / 3600 <= 24 
      THEN 'Priority Attention (12-24 hrs)'
    WHEN EXTRACT(EPOCH FROM (NOW() - o."createdAt")) / 3600 <= 48 
      THEN 'High Priority (24-48 hrs)'
...
WHERE
  ...
  AND COALESCE((o.meta_data ->> 'is_milk_distribution_order')::boolean, FALSE) = FALSE
  AND COALESCE((o.meta_data ->> 'is_water_distribution_order')::boolean, FALSE) = FALSE
